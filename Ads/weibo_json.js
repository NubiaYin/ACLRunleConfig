...
function removeAdBanner(e) {
    return e.data.close_ad_setting && delete e.data.close_ad_setting,
    e.data.detail_banner_ad && (e.data.detail_banner_ad = []),
    e
}
function removeAdPreload(e) {
    if (!e.ads) return e;
    for (let t of(e.last_ad_show_interval = 86400, e.ads)) t.start_time = 2681574400,
...
function removeSearchTopic(e) {
    return e.data && 0 !== e.data.search_topic ? .cards.length && (e.data.search_topic.cards = Object.values(e.data.search_topic.cards).filter(e = >"searchtop" != e.type), e.data.trending_topic && delete e.data.trending_topic),
    e
}
function modifiedUserCenter(e) {
    return e.data && 0 !== e.data.length && e.data.cards && (e.data.cards = Object.values(e.data.cards).filter(e = >"personal_vip" != e.items[0].type)),
    e
}
function removeTopics(e) {
    return e.data && (e.data.order = ["search_topic"]),
    e
}
function isAd(e) {
...
function removeMain(e) {
    if (e.loadedInfo && e.loadedInfo.headers && delete e.loadedInfo.headers, !e.items) return e;
    let t = [];
    for (let o of e.items) if ("feed" == o.category) isAd(o.data) || t.push(o);
    else if ("group" == o.category) {
        if (o.items.length > 0 && o.items[0].data ? .itemid ? .includes("search_input")) o.items = o.items.filter(e = >e ? .data ? .itemid ? .includes("mine_topics") || e ? .data ? .itemid ? .includes("search_input") || e ? .data ? .card_type == 202),
        o.items[0].data.hotwords = [{
            word: "搜索超话",
            tip: ""
        }],
        t.push(o);
            ...
            o.items.length > 0 && (o.items = Object.values(o.items).filter(e = >"feed" == e.category || "card" == e.category)),
            t.push(o)
        }
    } else {
...
function removeSearchMain(e) {
    ...
    return e.channelInfo.channels = o,
...
function removeSearch(e) {
    ...
    return e.items = t,
    e.loadedInfo && (e.loadedInfo.searchBarContent = [], e.loadedInfo.headerBack && (e.loadedInfo.headerBack.channelStyleMap = {})),
...
function removePage(e) {
    return removeCards(e),
    mainConfig.removePinedTrending && e.cards && e.cards.length > 0 && e.cards[0].card_group && (e.cards[0].card_group = e.cards[0].card_group.filter(e = >!(e ? .actionlog ? .ext ? .includes("ads_word") || e ? .itemid ? .includes("t:51") || e ? .itemid ? .includes("ads_word")))),
    e
}
...
function removeHomeVip(e) {
    return e.header && e.header.vipView && (e.header.vipView = null),
    e
}
...
function removeHome(e) {
    ...
    for (let o of e.items) {
        let i = o.itemId;
        if ("profileme_mine" == i) mainConfig.removeHomeVip && (o = removeHomeVip(o)),
        o.header ? .vipIcon && delete o.header.vipIcon,
        updateFollowOrder(o),
        t.push(o);
        else if ("100505_-_top8" == i) updateProfileSkin(o, "profileSkin1"),
        t.push(o);
            ...
            o.images ? .length > 0 && (o.images = o.images.filter(e = >"100505_-_chaohua" == e.itemId || "100505_-_recentlyuser" == e.itemId)),
            t.push(o)
        }
    }
    return e.items = t,
...
function containerHandler(e) {
    mainConfig.removeInterestFriendInTopic && "超话里的好友" === e.card_type_name && (log("remove 超话里的好友"), e.card_group = []),
    mainConfig.removeInterestTopic && e.itemid && (e.itemid.indexOf("infeed_may_interest_in") > -1 ? (log("remove 感兴趣的超话"), e.card_group = []) : e.itemid.indexOf("infeed_friends_recommend") > -1 && (log("remove 超话好友关注"), e.card_group = []))
}
...
function tabSkinHandler(e) {
    try {
        ...
        let o = e.data.list;
        for (let i of o) i.version = t,
        i.downloadlink = mainConfig.tabIconPath;
...
function removeLuaScreenAds(e) {
    ...
    for (let t of e.cached_ad.ads) t.start_date = 1893254400,
    t.show_count = 0,
    t.duration = 0,
...
function removePhpScreenAds(e) {
    if (!e.ads) return e;
    for (let t of(e.show_push_splash_ad = !1, e.background_delay_display_time = 0, e.lastAdShow_delay_display_time = 0, e.realtime_ad_video_stall_time = 0, e.realtime_ad_timeout_duration = 0, e.ads)) t.displaytime = 0,
